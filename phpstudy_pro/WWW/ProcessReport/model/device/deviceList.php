...
//业务逻辑
if($type=='SELECT_TY'){
    $query = "SELECT 序号 AS value, 大类 AS name FROM 设备资产大类表 ORDER BY 序号";
    echo GetData($pdo,$query);
}
if($type=='SELECT_BM'){
    $query = "SELECT 部门编号 AS value, 部门名称 AS name FROM 部门表 ORDER BY 部门编号";
    echo GetData($pdo,$query);
}
            ...
            for($i=0;$i <count($data); $i++){
                ...
                if(!empty($row) && $row['flg']=='insert'){
                  ...
                  /*  $inSql="INSERT INTO 设备资产表 (id,flg,设备编号,大类,名称,规格,单位,数量,金额,出厂时间,生产厂家,购置时间,规定使用年限,备注,记录人,"
                    ...
                    ."'". $BH."','".$row['大类']."','".$row['名称']."','".$row['规格']."','".$row['单位']."',"
                    ...
                    $inSql="INSERT INTO 设备资产表 (id,flg,设备编号,大类,名称,规格,单位,数量,生产厂家,购置时间,规定使用年限,备注,"
                    ."记录时间,使用部门,使用状态)VALUES ($inId,'',"
                    ."'". $BH."','".$row['大类']."','".$row['名称']."','".$row['规格']."','".$row['单位']."',"
                    ."'".$row['数量']."','".$row['生产厂家']."','".$row['购置时间']."',"
                    ."'".$row['规定使用年限']."','".$row['备注']."','".$row['记录时间']."',"
                    ."'".$row['使用部门']."','".$row['使用状态']."');";
                    ...
                    $sql= "UPDATE 设备资产表 SET 设备编号='".$BH."', 大类='".$row['大类']."',名称='".$row['名称']."',规格='".$row['规格']."',单位='".$row['单位']."',"
                    ."数量='".$row['数量']."',生产厂家='".$row['生产厂家']."',购置时间='".$row['购置时间']."',"
                    ." 规定使用年限='".$row['规定使用年限']."',备注='".$row['备注']."',记录时间='".$row['记录时间']."',使用部门='".$row['使用部门']."',使用状态='".$row['使用状态']."'"
