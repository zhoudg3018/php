    ...
    try{
      ...
      for($i = 0; $i < count($exfn); $i++) {
          ...
          if(!empty($arr)){
            ...
            门玻璃,洞口类型,门长,门宽,门厚,封边类型,门锁孔,合页槽,详单条码,机器码,序号,详单备注,组序号)VALUES(
            '$arr[0]','$arr[1]','$arr[2]','$arr[3]','$arr[4]','$arr[5]','$arr[6]','$arr[7]','$arr[8]','$arr[9]','$arr[10]',
            '$arr[11]','$arr[12]','$arr[13]','$arr[14]','$arr[15]','$arr[16]','$arr[17]','$arr[18]','$arr[19]','$arr[20]','$arr[21]','$ID',$i+1,'$arr[22]','$arr[23]');";
            $conn->exec($sql);//预处理
          }
      }
      //提交事务
